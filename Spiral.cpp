...
Spiral::Spiral(const Vec3F& origin, size_t numOfPoint, size_t radiusStep) : m_numOfPoints{ numOfPoint }, m_endDraw{ numOfPoint } {
	...
	for (int i = 0; i < m_numOfPoints; ++i) {
		...
		point.x = (m_origin.x + (m_angleDir * std::cosf(radianAngle) * m_radius));
		point.y = (m_origin.y + (m_angleDir * std::sinf(radianAngle) * m_radius));

		if (i == m_numOfPoints / 2) {
			m_origin = { point.x * 2, point.y * 2, 0.f };
